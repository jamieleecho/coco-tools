...
110 T$="flipping"+CHR$(127)+"bits":TL=0:GOSUB9600
111 L=69:PRINT@L,"board";:PRINT@L+15,"target";
113 BC$=CHR$(124):BX=3:BY=2:BH=10:BW=10:GOSUB9400
...
240 IF W=1 THEN T$="well"+CHR$(127)+"done":TL=15:GOSUB9600:GOSUB9700:LE=LE+1:M=0:W=0:?@480,W$;:GOSUB1000:GOTO170
250 W=0:?@480,W$;
...
305 FOR Y=1TO7
310 FOR X=1TO7
315 IF P(X,Y)=0 THEN PRINT@S+Y*32+X,CHR$(127); ELSE PRINT@S+Y*32+X,CHR$(207);
320 NEXT:NEXT
340 RETURN
...
355 FOR Y=1TO7
360 FOR X=1TO7
370 IF G(X,Y)=0 THEN PRINT@S+Y*32+X,CHR$(127); ELSE PRINT@S+Y*32+X,CHR$(207);
380 NEXT:NEXT
390 RETURN
...
810 T$="please"+CHR$(127)+"wait":TL=15:GOSUB9600
...
1010 PRINT@448,W$;:
1020 PRINT@451,"level"+STR$(LE);:?@456,CHR$(127);
1030 PRINT@467,"moves"+STR$(M);:?@472,CHR$(127);
1090 RETURN
...
9930 BC$=CHR$(127):?@483,"press"+BC$+"any"+BC$+"key"+BC$+"to"+BC$+"continue";:GOSUB9810:RETURN

9950 REM LOWER CASE TEXT
9960 TL=LEN(T$):O$=""
9970 FORBR=1TOTL:C$=MID$(T$,BR,1):BH=ASC(C$):IF BH<91 AND BH>64 THEN C$=CHR$(BH+32)
9971 IFBH=32THENC$=CHR$(127)
9975 O$=O$+C$
9979 NEXT
9980 T$=O$
9999 RETURN
